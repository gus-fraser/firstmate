...
1. **Resolve the profile.**
   ...
   A recorded [Claude account](configuration.md#claude-accounts-configclaude-accounts) is kept on a Claude relaunch and dropped on a switch off Claude, and one that no longer resolves refuses before the checkpoint.
2. **Safe checkpoint.**
